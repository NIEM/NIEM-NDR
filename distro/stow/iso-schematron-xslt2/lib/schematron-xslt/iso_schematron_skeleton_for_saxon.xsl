...
<xsl:template 
	...
	<axsl:stylesheet
	   ...
	   >
	    ...
	    <xsl:copy-of select="xsl:import-schema"/>
...
<xsl:template match="*" mode="stylesheetbody">
   ...
   for ZIP archives.
	...
	<xsl:apply-templates select="*[not(self::iso:ns) 
				       and not(self::xsl:import-schema)]"/>
</xsl:template>
...
<xsl:template name="generate-default-rules">
		...
		<xsl:comment>This mode can be used to generate an ugly though full XPath for locators</xsl:comment><xsl:text>&#10;</xsl:text>
		  ...
		  <axsl:text>/*</axsl:text>
                  <axsl:if test="not(parent::document-node())">
                    <axsl:text>[</axsl:text>		
		...
		<axsl:template match="@*" mode="schematron-get-full-path">
		  <axsl:apply-templates select="parent::*" mode="schematron-get-full-path"/>
      		  <axsl:text>/@*[string(namespace-uri()) = '</axsl:text>
                  <axsl:value-of select="namespace-uri()"/>
                  <axsl:text>' and local-name() = '</axsl:text>
