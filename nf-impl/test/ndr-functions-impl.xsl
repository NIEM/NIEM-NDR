<?xml version="1.0" encoding="UTF-8"?><stylesheet xmlns:catalog="urn:oasis:names:tc:entity:xmlns:xml:catalog" xmlns:ct="http://release.niem.gov/niem/conformanceTargets/3.0/" xmlns:extra="http://example.org/extra-xsl" xmlns:impl="http://example.org/naming-and-design-rules-3.0-functions-impl" xmlns:saxon="http://saxon.sf.net/" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <param name="xml-catalog" as="xs:anyURI"/>
  ...
  <function name="impl:resolve-namespace" as="element(xs:schema)?">
    ...
    <choose>
      <when test="not(doc-available($xml-catalog))">
        <message>
          <value-of select="impl:get-location($context)"/>
          <text>: impl:resolve-namespace(</text>
          <value-of select="node-name($context)"/>
          <text>, </text>
          <value-of select="$namespace-uri"/>
          <text>): can't resolve xml-catalog (</text>
          <value-of select="$xml-catalog"/>
          <text>).</text>
        </message>
        <sequence select="()"/>
      </when>
      <otherwise>
        <variable name="catalog-element" as="element(catalog:catalog)?" select="doc($xml-catalog)/catalog:catalog"/>
        <choose>
  ...
  <function name="impl:has-effective-conformance-target" as="xs:boolean">
    <param name="context" as="element()"/>
  ...
  <function name="impl:get-location" as="xs:string">
    ...
    <template match="catalog:nextCatalog" mode="impl:resolve-namespace" as="element(xs:schema)?">
      <param name="namespace-uri" as="xs:anyURI" tunnel="yes"/>
      <variable name="catalog-uri" as="xs:anyURI" select="xs:anyURI(@catalog)"/>
      <choose>
        ...
        <when test="doc-available($namespace-uri)">
          <variable name="catalog" as="element(catalog:catalog)?" select="doc($namespace-uri)/catalog:catalog"/>
          <choose>
            <when test="exists($catalog)">
              <apply-templates mode="impl:resolve-namespace" select="$catalog"/>
            </when>
            <otherwise>
              <message>
                <value-of select="impl:get-location(.)"/>
                <text>: catalog:nextCatalog/@catalog does not resolve to catalog.</text>
              </message>
              <sequence select="()"/>
            </otherwise>
          </choose>
