    ...
    <section id="section-use-of-namespaces">
      ...
      <section><title>Use of normative Schematron</title>
        ...
        <p>An execution of a Schematron pattern that issues a report (represented via <qName>sch:report</qName>)
          indicates a warning. This may be:</p>

          <ul>
            <li><p>An indication that the automated rules are not sufficient to validate a conformance rule, and
                that another means is required to ensure conformance. This is frequently a reference to another
                specification. For example, see <ref idref="target-ns-is-rfc3986"/>, which references
              <ref idref="RFC3986"/>.</p></li>

            <li><p>an indication that an automated rule has found that the
                assessed artifact violates a recommendation of the specification
                (i.e., a <q>SHOULD</q>, rather than a <q>MUST</q>), and that attention should
                be paid to ensure that the artifact maintains the spirit of the
                specification. For example, see <ref idref="el-representation-is-abstract"/>.</p></li>

          </ul>

          <p>In addition to the exclusive use of <qName>sch:report</qName> for warnings, this document annotates
            <qName>sch:report</qName> elements with attribute <code>role="warning"</code>, which is interpreted
            by some Schematron platforms to indicate that a failure indicates a warning, rather than an
            error.</p>
  ...
  <section>
    ...
    <section>
      ...
      <section>
        ...
        <p>NIEM relies heavily on XML namespaces to prevent naming conflicts and clashes. Reuse of any component is always by reference to both its namespace and its local name. All NIEM component names have global scope. Therefore, validation always occurs against the reference schemas or subsets thereof.</p>
        <p>Example:</p>
  ...
  <sch:rule context="xs:simpleContent">
    <sch:assert test="exists(xs:extension)"
      >A complex type definition with simple content schema component MUST have a derivation method of extension.</sch:assert>
            ...
            extension. This allows for the above cases while disallowing more complicated syntactic options
            available in XML Schema. The above rule allows for use of <qName>xs:restriction</qName>
